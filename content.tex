...
%
The goal of this work is to simplify the program $\to$ model process.
%
...
%
Higher-order recursion schemes (HORS) are a powerful model for simply typed functional programs with good model-checking properties~\citep{dvanhorn:Ong2006ModelChecking}, but the program $\to$ HORS process is narrowly applicable.
%
...
%%
When striving for precision, the interconnectedness of data-flow and control-flow analysis in higher-order languages extend upwards to the interconnectedness of model construction and model-checking.
%
...
%
The resulting artifacts can be used as models in the first-order tools~\citep{ianjohnson:bandera}
...
\begin{align*}
  \tpl{\mval,\mstore,\mframe,\kcons{\kfn{\vclo{\slam{\mvar}{\mexpr}}{\menv}}}{\mkont},\mktab,\mmemo} &\stepto
  \tpl{\mpoint,
    \mstore',
    \mframe',
    \krt{\mpoint, \mstore'},
...
%
\paragraph{Problem:} now $\rt$ continuations contain a store, and captured continuations can appear in the store.
%

...
return with the result of the call, with the resulting context changes---in this case only the store (the second case of rule 4).
...
table $\mmemo$. The memo table need only map to values, since the
global store will always subsume the store that would otherwise be
mapped. All the states in the frontier are stepped with the current
store, after which the next store is the least upper bound of all the
resulting stores. The next frontier contains only states resulting
from stepping the previous frontier that we haven't seen at this next
store. Systematic techniques for a performant implementation can be
found in \citet{ianjohnson:oaam:icfp2013}.
...
\begin{conjecture}
  Tabular semantics has a bisimulation with the standard semantics with a sound allocation strategy.
\end{conjecture}
\begin{proof}
...
\begin{conjecture}
  Store-widened tabular semantics has a bisimulation with store/memo/k-table-widened semantics.
\end{conjecture}
...
falling back on the heap (of course this must be limited to immutable addresses).
...
% Return
   ...
   \\
   where & $\mmemo' = \memoize(\mstore,\mmemo, \mktab, \mctx, \mval)$
  \end{tabular}
...
\begin{align*}
  \memoize(\mstore,\mmemo, \mktab, ((\mexpr,\menv), \mstore'), \mval) &=
  \joinone{\mmemo}{((\mexpr,\menv), \mstore')}{(\mval,\mstore')} \\
  \memoize(\mstore,\mmemo, \mktab, ((\mexpr,\menv), \maddr), \mval) &=
  \mmemo \sqcup \bigsqcup\limits_{\mstore' \in \mktab(\maddr)}\snglm{((\mexpr,\menv), \mstore')}{(\mval,\mstore)}
\end{align*}
...
%% TODO: complete
\paragraph{Comparison to CFA2 CPS'd to remove {\tt shift} and {\tt reset}:}{

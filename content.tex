...
\begin{align*}
  ...
  \mkframe \in \Frame &::= \kwcmk{\mexpr, \mexpr, \menv} \alt
                           ...
                           \kcmslk{\mval} \alt \ldots \\ 
  \mkont \in \Kont &::= \kmt^\mmarks \alt \krt{\mctx} \alt \kcons{\mkframe^\mwmarks}{\mkont} \\
  \mmarks \in \Marks &= \Value \to \wp(\Value) \\
  \mwmarks \in \QualifiedMarks &::= \scopy{\mmarks} \alt \sext{\mmarks} \\
...
\begin{figure}
  ...
  \begin{tabular}{r|l}
    \hline
% wcm eval key
    $\tpl{(\swcm{\mexpr_k}{\mexpr_v}{\mexpr}, \menv), \mstore, \mkont, \mktab, \mmemo}$
    &
    $\tpl{(\mexpr_k, \menv), \mstore, \kcons{\kwcmk{\mexpr_v,\mexpr,\menv}}{\mkont}, \mktab, \mmemo}$
    \\
% wcm eval value
    $\tpl{\mval_k, \mstore, \kcons{\kwcmk{\mexpr_v,\mexpr,\menv}}{\mkont}, \mktab, \mmemo}$
    &
    $\tpl{(\mexpr_v, \menv), \mstore, \kcons{\kwcmv{\mval_k,\mexpr,\menv}}{\mkont}, \mktab, \mmemo}$
    \\    
% wcm all done.
...
% function call
    $\tpl{\mval,\mstore,\kcons{\kfn{\vclo{\slam{\mvar}{\mexpr}}{\menv}}}{\mkont}}$
    &
    $\tpl{(\mexpr, \menv'), \mstore', \krt{\mctx},\mktab',\mmemo}$
...
% Current continuation marks
    ...
    \\
% cms->list mark set
    $\tpl{(\scmsl{\mexpr_\mmarks}{\mexpr_k}, \menv), \mstore, \mkont, \mktab, \mmemo}$
    &
    $\tpl{(\mexpr_\mmarks, \menv), \mstore, \kcons{\kcmsls{\mexpr_k,\menv}}{\mkont}, \mktab, \mmemo}$
    \\
% cms->list key
    $\tpl{\mval, \mstore, \kcons{\kcmsls{\mexpr_k,\menv}}{\mkont}, \mktab, \mmemo}$
    &
    $\tpl{(\mexpr_k, \menv), \mstore, \kcons{\kcmslk{\mval}}{\mkont}, \mktab, \mmemo}$
    \\
% cms->list done
...
\\[2pt]
  \domarkstep(\krt{\mctx}) &= (\krt{\rewritectx(\mctx)}, \mctx) \\
  \domarkstep(\kcons{\mkframe^\mmarks}{\mkont}) &= (\kcons{\mkframe^{\mmarks \sqcup_{!{}} [\mval_k \mapsto \mval_v]}}{\mkont}, \bot) \\
  \domarkstep(\kmt^\mmarks) &= (\kmt^{\mmarks \sqcup_{!{}} [\mval_k \mapsto \mval_v]}, \bot)
\end{align*}

Second, the $\domarkset$ metafunction builds a map of keys to lists, and builds a private store for the list constructions.
%
For this, we extend $\alloc$'s specification to allow a second argument, the private store, and produce a pair of addresses.
%
...
\begin{align*}
  \domarkset(\mstate\equiv\tpl{\mpoint,\mstore,\mkont,\mktab,\mmemo}) &=
    (\maddr_a, \maddr_d) = \alloc(\mstate,\mstore')
\end{align*}
}

...
\begin{claim}(Correctness of PDCFA)\label{thm:pdcfa-tabular}
...
\begin{claim}[Equivalence with global tables]\label{thm:global}
...
%
\begin{claim}[No dangling pointers]\label{thm:gc-pointers}
  All addresses in $\Gamma(\mstate)$ are in the domain of $\Gamma(\mstate).\mstore$.
\end{claim}

\begin{claim}[Garbage is irrelevant]\label{thm:gc-concrete}
  If $\mstate \stepto^n \mstate'$ and $\mstate \stepto^n \mstate''$ then $\Gamma(\mstate') = \Gamma(\mstate'')$.
\end{claim}

\begin{claim}[Abstract GC is sound for garbage-free states]\label{thm:gc-sound}
...
\begin{claim}[Correctness of stack frame refinement]\label{thm:refinement}
...
\begin{claim}[Correctness of CFA2]\label{thm:cfa2}

...
\begin{figure}
  \centering
    \begin{minipage}{0.55\linewidth}
      \begin{center}
        $\begin{array}{l}\includegraphics[scale=0.65]{example1}\end{array}$ \\
        $\Downarrow$ \\
        $\begin{array}{l}\includegraphics[scale=0.65]{example2}\end{array}$ \\
        $\Downarrow$ \\
        $\begin{array}{l}\includegraphics[scale=0.65]{example3}\end{array}$
      \end{center}
    \end{minipage}
...
% Return
   ...
   \\
   where & $\mmemo' = \mmemo \sqcup \memoize(\mmktab, \mctx, (\mval, \mstore, \mmktab))$
  \end{tabular}
...
\begin{align*}
  \returns(\mmktab, \mktab, ((\mexpr,\menv), \mstore)) &= \mktab((\mexpr,\menv), \mstore) \\
  \returns(\mmktab, \mktab, ((\mexpr,\menv), \maddr)) &=
    \bigcup\setbuild{\mktab((\mexpr,\menv),\mstore)}{\mstore \in \mmktab(\maddr)}
\end{align*}

At return boundaries, the memo table must add the result to all the represented contexts:
\begin{align*}
  \memoize(\mmktab, ((\mexpr,\menv), \mstore'), \mathit{out}) &=
  [((\mexpr,\menv), \mstore') \mapsto \set{{\mathit{out}}}] \\
  \memoize(\mmktab, ((\mexpr,\menv), \maddr), \mathit{out}) &=
  \bigsqcup\limits_{\mstore' \in \mmktab(\maddr)}\snglm{((\mexpr,\menv), \mstore')}{\mathit{out}}
\end{align*}
...
\begin{align*}
  ...
  \\[2pt]
   \addstore(\kmt) &= \mmktab \\
   \addstore(\krt{((\mexpr,\menv),\mstore)}) &= \joinone{\mmktab}{\maddr}{\mstore} \\

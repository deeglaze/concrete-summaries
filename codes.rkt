...
(define (example stage)
  (define-values (rt0x rt0y) (values -23 -18))
  (define-values (rt1x rt1y) (values -45 -44))
  (define-values (M0x M0y) (values 5 0))
  (define-values (M1x M1y) (values 30 10))
  (define base
  ...
  (define call0
    (pin-arrow-label-line (rotate (code (rt ctx₀)) (* 1/6 pi))
                          15
                          base
                          (find-tag base 'id0)
                          lc-find
  ...
  (define cont0
    (pin-arrow-line 8 return0
                    ...
                    #:style 'dot))
  (define call1
    (pin-arrow-label-line (rotate (code (rt ctx₁)) (* 0.28 pi))
                          15
                          cont0
                          (find-tag base 'id1)
                          lc-find
...
(with-output-to-file (format "example~a.pdf" i) #:exists 'replace (λ () (write-bytes (convert (example i) 'pdf-bytes)))))
#|

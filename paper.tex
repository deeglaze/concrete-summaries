...
% the $\rt$ continuation, we introduced a circularity in the data
% represented in our machine, which can lead to a non-terminating
% analysis --- this is precisely what the AAM approach avoids. There is
% currently no known model of computation that allows stack capture and
...
% continuation, possibly with some gaps, after which we say that any
% continuation matching that ``outline'' is acceptable. The amount to
% represent is a tunable parameter; we can represent the continuation up
% to boundedly-many function calls, or have innermost prompt tags
% matching $\many\mtag$ for boundedly-long $\many\mtag$ (or a mix of
% $dw$ frames and prompt tags), etc.

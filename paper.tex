...
analyses for higher-order languages could not handle return flow precisely
...
This paper gives a common, simple framework to discuss both CFA2 and
PDCFA along with a recipe to apply pushdown analysis techniques to an
off-the-shelf semantics in a manner similar to
~\citet{dvanhorn:VanHorn2010Abstracting} (a technique called
abstracting abstract machines, or AAM). We then extend the state of
the art by giving a novel analysis for delimited, composable control. \\
...
you too can write a pushdown analysis for your language. In order to
demonstrate the applicability of this viewpoint, we show a new
analysis for a language with composable control. All of the semantics
modeled in this paper are implemented in full detail in PLT Redex and
available at
...
as concrete machinery in the programming language semantics: breaking
the recursive structure of continuations by indirecting them through a
table with appropriately precise keys, and memoization.  Once the
machine semantics is in this form, simple pointwise abstraction leads
to the summarization algorithms that we see in the literature. In
effect, we give a \emph{concrete} semantics to pushdown analysis.
